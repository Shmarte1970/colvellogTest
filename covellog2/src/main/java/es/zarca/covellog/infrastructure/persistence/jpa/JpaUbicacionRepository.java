...
import es.zarca.covellog.domain.model.ubicacion.exception.UbicacionNotExistException;
 ...
 */
@ApplicationScoped
public class JpaUbicacionRepository implements UbicacionRepository, Serializable {
    private static final long serialVersionUID = 1L;

    private static final Logger LOGGER = Logger.getLogger(JpaUbicacionRepository.class.getName());

    @PersistenceContext(unitName = "covellog2-pu")
    private EntityManager entityManager;

    @Override
    public Ubicacion find(Integer id) {
        Ubicacion almacen;
        try {
            almacen = entityManager.createNamedQuery("Ubicacion.findById", Ubicacion.class).setParameter("id", id).getSingleResult();
    ...
    @Override
    public Ubicacion findOrFail(Integer id) {
        Ubicacion almacen = find(id);
        if (almacen == null) {
            throw new UbicacionNotExistException(id);
        }
        return almacen;
    }
    ...
    private List<Order> processOrder(CriteriaBuilder cb, Root<Ubicacion> root, Ordre ordre) {
        Path<String> path;
        List<Order> orderList = new ArrayList();
        switch (ordre.getId()) {
            case "empresa.nombre":
                Join<Ubicacion, Empresa> empresa = root.join("empresa");
            ...
            case "direccion.poblacion.nom":
                Join<Ubicacion, Poblacio> poblacio = root.join("direccion").join("poblacion");
    ...
    private List<Predicate> processFilters(CriteriaQuery query, CriteriaBuilder cb, Root<Ubicacion> root, Map<String, Object> filters) {
    ...
    @Override
    public List<Ubicacion> find(int first, int pageSize, Ordre sortOrder, Map<String, Object> filters) {
        ...
        try {
            ...
            CriteriaQuery<Ubicacion> select = criteriaQuery.select(root);
            ...
            TypedQuery<Ubicacion> query = entityManager.createQuery(select);
            query.setFirstResult(first);
            query.setMaxResults(pageSize);
            List<Ubicacion> list = query.getResultList();
            return list;
        } catch (Exception ex) {
    ...
    public List<Ubicacion> findAll(){      
        return entityManager.createNamedQuery("Ubicacion.findAll", Ubicacion.class).getResultList();
    }

    @Override
    public int findTotalCount() {
        Crono crono = new Crono();               
        Query query = entityManager.createQuery("Select count(f.id) From Ubicacion f");
