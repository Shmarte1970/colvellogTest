...
import es.zarca.covellog.application.stock.exception.FlotaNotExistException;
 ...
 */
@ApplicationScoped
public class JpaFlotaRepository implements FlotaRepository, Serializable {
    private static final long serialVersionUID = 1L;

    private static final Logger logger = Logger.getLogger(JpaFlotaRepository.class.getName());

    @PersistenceContext(unitName = "covellog2-pu")
    private EntityManager entityManager;

    @Override
    public Flota find(String id) {
        Flota flota;
        try {
            flota = entityManager.createNamedQuery("Flota.findById", Flota.class).setParameter("id", id).getSingleResult();
    ...
    @Override
    public Flota findOrFail(String id) {
        Flota flota = find(id);
        if (flota == null) {
            throw new FlotaNotExistException(id);
        }
        return flota;
    }
    ...
    private List<Order> processOrder(CriteriaBuilder cb, Root<Flota> root, Ordre ordre) {
        Path<String> path;
    ...
    private List<Predicate> processFilters(CriteriaQuery query, CriteriaBuilder cb, Root<Flota> root, Map<String, Object> filters) {
    ...
    public List<Flota> findAll() {
        List<Flota> list = entityManager.createNamedQuery("Flota.findAll", Flota.class).getResultList();
        return list;
    }
    
    @Override
    public List<Flota> find(int first, int pageSize, Ordre sortOrder, Map<String, Object> filters) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        CriteriaQuery<Flota> criteriaQuery = cb.createQuery(Flota.class);
        Root<Flota> root = criteriaQuery.from(Flota.class);
        CriteriaQuery<Flota> select = criteriaQuery.select(root);
        ...
        TypedQuery<Flota> query = entityManager.createQuery(select);
        query.setFirstResult(first);
        query.setMaxResults(pageSize);
        List<Flota> list = query.getResultList();
        return list;
    }
    ...
    public int findTotalCount() {
        ...
        Query query = entityManager.createQuery("Select count(f.id) From Flota f");
    ...
    @Override
    public void store(Flota flota) {
        try {
            entityManager.persist(flota);
    ...
    @Override
    public void remove(Flota flota) {
        try {
            entityManager.remove(entityManager.merge(flota));
        } catch (ConstraintViolationException ex) {
            throw new PersistenceException(ex);
        }
    }
