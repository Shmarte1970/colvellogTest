 ...
 */
@ApplicationScoped
public class JpaAlmacenRepository implements AlmacenRepository, Serializable {
    private static final long serialVersionUID = 1L;

    private static final Logger LOGGER = Logger.getLogger(JpaAlmacenRepository.class.getName());
    ...
    public Almacen find(Integer id) {
        Almacen almacen;
        try {
            almacen = entityManager.createNamedQuery("Almacen.findById", Almacen.class).setParameter("id", id).getSingleResult();
    ...
    @Override
    public Almacen findOrFail(Integer id) {
    ...
    private List<Order> processOrder(CriteriaBuilder cb, Root<Almacen> root, Ordre ordre) {
        Path<String> path;
        List<Order> orderList = new ArrayList();
        switch (ordre.getId()) {
            case "empresa.nombre":
                Join<Almacen, Empresa> empresa = root.join("empresa");
            ...
            case "direccion.poblacion.nom":
                Join<Almacen, Poblacio> poblacio = root.join("direccion").join("poblacion");
    ...
    private List<Predicate> processFilters(CriteriaQuery query, CriteriaBuilder cb, Root<Almacen> root, Map<String, Object> filters) {
    ...
    @Override
    public List<Almacen> find(int first, int pageSize, Ordre sortOrder, Map<String, Object> filters) {
        FunctionLog log = new FunctionLog(Capa.REPOSITORY);
        try {
            CriteriaBuilder cb = entityManager.getCriteriaBuilder();
            CriteriaQuery<Almacen> criteriaQuery = cb.createQuery(Almacen.class);
            Root<Almacen> root = criteriaQuery.from(Almacen.class);
            CriteriaQuery<Almacen> select = criteriaQuery.select(root);
            ...
            TypedQuery<Almacen> query = entityManager.createQuery(select);
            query.setFirstResult(first);
            query.setMaxResults(pageSize);
            List<Almacen> list = query.getResultList();
            return list;
        } catch (Exception ex) {
    ...
    public List<Almacen> findAll(){      
        return entityManager.createNamedQuery("Almacen.findAll", Almacen.class).getResultList();
    }

    @Override
    public int findTotalCount() {
        Crono crono = new Crono();               
        Query query = entityManager.createQuery("Select count(f.id) From Almacen f");
    ...
    @Override
    public void store(Almacen almacen) {
        try {
    ...
    @Override
    public Almacen findByUsername(String username) {
        Almacen almacen;
        try {
            almacen = entityManager.createNamedQuery("Almacen.findByUsername", Almacen.class).setParameter("username", username).getSingleResult();
        } catch (NoResultException e) {
            almacen = null;
        }
        return almacen;
    ...
    @Override
    public List<Almacen> findAll(){      
        Query query = entityManager.createNamedQuery("Almacen.findAll", Almacen.class);
        List <Almacen> list = query.getResultList();
        return list;
    }
    
    @Override
    public List<Almacen> findAll(int start, int size){      
        Query query = entityManager.createNamedQuery("Almacen.findAll", Almacen.class);
    ...
    public int findAllTotalCount() {
        Query query = entityManager.createQuery("Select count(o.id) From Almacen o");
    ...
    @Override
    public List<Almacen> findAll( Ordre ordre, Map<String, Object> filters) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        CriteriaQuery<Almacen> criteriaQuery = cb.createQuery(Almacen.class);
        Root<Almacen> root = criteriaQuery.from(Almacen.class);
        CriteriaQuery<Almacen> select = criteriaQuery.select(root);
    ...
    @Override
    public List<Almacen> findAll(int start, int size, Ordre ordre, Map<String, Object> filters) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        CriteriaQuery<Almacen> criteriaQuery = cb.createQuery(Almacen.class);
        Root<Almacen> root = criteriaQuery.from(Almacen.class);
        CriteriaQuery<Almacen> select = criteriaQuery.select(root);
