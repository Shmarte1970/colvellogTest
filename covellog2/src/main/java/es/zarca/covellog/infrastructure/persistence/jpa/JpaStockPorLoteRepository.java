...
import es.zarca.covellog.domain.model.stock.exception.StockPorLoteNotExistException;
 ...
 */
@ApplicationScoped
public class JpaStockPorLoteRepository implements StockPorLoteRepository, Serializable {
    private static final long serialVersionUID = 1L;

    private static final Logger logger = Logger.getLogger(JpaStockPorLoteRepository.class.getName());

    @PersistenceContext(unitName = "covellog2-pu")
    private EntityManager entityManager;

    @Override
    public StockPorLote find(String id) {
        StockPorLote stockPorLote;
        try {
            stockPorLote = entityManager.createNamedQuery("StockPorLote.findById", StockPorLote.class).setParameter("id", id).getSingleResult();
    ...
    @Override
    public StockPorLote findOrFail(String id) {
        StockPorLote stockPorLote = find(id);
        if (stockPorLote == null) {
            throw new StockPorLoteNotExistException(id);
        }
        return stockPorLote;
    }
    ...
    private List<Order> processOrder(CriteriaBuilder cb, Root<StockPorLote> root, Ordre ordre) {
        Path<String> path;
    ...
    private List<Predicate> processFilters(CriteriaQuery query, CriteriaBuilder cb, Root<StockPorLote> root, Map<String, Object> filters) {
    ...
    @Override
    public List<StockPorLote> find(int first, int pageSize, Ordre sortOrder, Map<String, Object> filters) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        CriteriaQuery<StockPorLote> criteriaQuery = cb.createQuery(StockPorLote.class);
        Root<StockPorLote> root = criteriaQuery.from(StockPorLote.class);
        CriteriaQuery<StockPorLote> select = criteriaQuery.select(root);
        ...
        TypedQuery<StockPorLote> query = entityManager.createQuery(select);
        query.setFirstResult(first);
        query.setMaxResults(pageSize);
        List<StockPorLote> list = query.getResultList();
        return list;
    }
    
    @Override
    public List<StockPorLote> findAll() {
        FunctionLog log = new FunctionLog(Capa.FACADE);
        try { 
            Query query = entityManager.createNamedQuery("StockPorLote.findAll", StockPorLote.class);
            List <StockPorLote> list = query.getResultList();
            return list;
    ...
    public int findTotalCount() {
        ...
        Query query = entityManager.createQuery("Select count(f.id) From StockPorLote f");
