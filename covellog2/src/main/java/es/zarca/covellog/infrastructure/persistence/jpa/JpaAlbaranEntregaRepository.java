...
import es.zarca.covellog.domain.model.albaran.exception.AlbaranEntregaNotExistException;
 ...
 */
@ApplicationScoped
public class JpaAlbaranEntregaRepository implements AlbaranEntregaRepository, Serializable {
    private static final long serialVersionUID = 1L;

    private static final Logger LOGGER = Logger.getLogger(JpaAlbaranEntregaRepository.class.getName());

    @PersistenceContext(unitName = "covellog2-pu")
    private EntityManager entityManager;

    @Override
    public AlbaranEntrega find(Integer id) {
        AlbaranEntrega almacen;
        try {
            almacen = entityManager.createNamedQuery("AlbaranEntrega.findById", AlbaranEntrega.class).setParameter("id", id).getSingleResult();
    ...
    @Override
    public AlbaranEntrega findOrFail(Integer id) {
        AlbaranEntrega almacen = find(id);
        if (almacen == null) {
            throw new AlbaranEntregaNotExistException(id);
        }
        return almacen;
    }
    ...
    private List<Order> processOrder(CriteriaBuilder cb, Root<AlbaranEntrega> root, Ordre ordre) {
        Path<String> path;
        List<Order> orderList = new ArrayList();
        switch (ordre.getId()) {
            case "empresa.nombre":
                Join<AlbaranEntrega, Empresa> empresa = root.join("empresa");
            ...
            case "direccion.poblacion.nom":
                Join<AlbaranEntrega, Poblacio> poblacio = root.join("direccion").join("poblacion");
    ...
    private List<Predicate> processFilters(CriteriaQuery query, CriteriaBuilder cb, Root<AlbaranEntrega> root, Map<String, Object> filters) {
    ...
    @Override
    public List<AlbaranEntrega> find(int first, int pageSize, Ordre sortOrder, Map<String, Object> filters) {
        FunctionLog log = new FunctionLog(Capa.REPOSITORY);
        try {
            CriteriaBuilder cb = entityManager.getCriteriaBuilder();
            CriteriaQuery<AlbaranEntrega> criteriaQuery = cb.createQuery(AlbaranEntrega.class);
            Root<AlbaranEntrega> root = criteriaQuery.from(AlbaranEntrega.class);
            CriteriaQuery<AlbaranEntrega> select = criteriaQuery.select(root);
            ...
            TypedQuery<AlbaranEntrega> query = entityManager.createQuery(select);
            query.setFirstResult(first);
            query.setMaxResults(pageSize);
            List<AlbaranEntrega> list = query.getResultList();
            return list;
        } catch (Exception ex) {
    ...
    @Override
    public List<AlbaranEntrega> findAll(){      
        return entityManager.createNamedQuery("AlbaranEntrega.findAll", AlbaranEntrega.class).getResultList();
    }

    @Override
    public int findTotalCount() {
        Crono crono = new Crono();               
        Query query = entityManager.createQuery("Select count(f.id) From AlbaranEntrega f");
    ...
    @Override
    public void store(AlbaranEntrega almacen) {
        try {
    ...
    @Override
    public void remove(AlbaranEntrega almacen) {
        try {
